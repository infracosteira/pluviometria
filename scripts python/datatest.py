import pandas as pd
from supabase import create_client, Client
from dotenv import load_dotenv
import os

load_dotenv()

supabase_url = os.getenv("SUPABASE_URL")
supabase_key = os.getenv("SUPABASE_KEY")

print("Chave e url copiados com sucesso!")

supabase: Client = create_client(supabase_url, supabase_key)

def upload_dataframe_to_supabase(dataframe: pd.DataFrame, table_name: str):
   
    data = dataframe.to_dict(orient="records")
    # Criar a tabela no Supabase
    try:
        columns = [{"name": col, "type": "text"} for col in dataframe.columns]
        supabase.table(table_name).create_table(columns).execute()
        print(f"Tabela '{table_name}' criada com sucesso.")
    except Exception as e:
        print(f"Tabela '{table_name}' já existe ou ocorreu um erro ao criá-la: {e}")

    # Inserir os dados no Supabase
    try:
        response = supabase.table(table_name).insert(data).execute()
        if response.get('status_code') == 201:
            print(f"Dados importados com sucesso para a tabela '{table_name}'")
        else:
            print(f"Erro ao importar dados: {response}")
    except Exception as e:
        print(f"Ocorreu um erro ao inserir os dados: {e}")

df = pd.read_csv('data/maindatabase.csv')
table_name = "pluviometria"

# Importar o DataFrame
upload_dataframe_to_supabase(df, table_name)
